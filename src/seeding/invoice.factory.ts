...
export const InvoiceFactory = setSeederFactory(Invoice, (faker) => {
    ...
    const subtotal = faker.number.float({ min: 50, max: 10000, fractionDigits: 2 });
    const tax = subtotal * 0.1; // 10% tax
    const discount = faker.number.float({ min: 0, max: subtotal * 0.15, fractionDigits: 2 });
    const total = subtotal + tax - discount;
    const amountPaid = faker.helpers.arrayElement([
        0,
        total * 0.5,
        total
    ]);
